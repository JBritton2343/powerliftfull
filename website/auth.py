    ...
    if request.method == "POST":
        ...
        if user:
            if check_password_hash(User.password, password):
                flash("Welcome, back ${user.name}", category="success")
                login_user(user, remember=True)

                return redirect(url_for('views.home'))
            else:
                flash("please type password again", category="error")
        else: 
            flash("Please, retype email address", category="error")
...
def signup():
    if request.method=="POST":
        ...
        user = User.query.filter_by(email=email).first()
        if user: 
            flash("Email already on file", category="error")
